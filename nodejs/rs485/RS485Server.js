...
var serialIO = new RS485IO(SUPPORTED_MESSAGE_VERSION, config.brokerPort, sensorConfig.addresses, sensorConfig.interfaces);

var RS485Server = function(callback) {
    this.http = require('http');
    this.message = new RS484Message;
    this.openArduinoPort(config.serialPort, config.baudrate, callback);
    serialIO.setAddress(config.RS485Address);
};
 ...
 * Call to close the serial port
 */
RS485Server.prototype.openArduinoPort = function (serialCOMPort, baudrate, callback) {
    var that = this;
    serialIO.openArduinoPort(serialCOMPort, baudrate, function() {
        ...
        serialIO.serialPort.on('data', function (stream) {
            try {
                while (that.readAndStoreMessages(stream, callback)) {};
            }
            catch (err) {
                console.log(err);
            }
 ...
 */
RS485Server.prototype.readAndStoreMessages = function (stream, callback) {
    var that = this;
...
RS485Server.prototype.sendMessage = function(targetAddress, key, value, reason) {
    ...
    reason = [reason].push({action: "http put", timestamp: new RS484Message().getLocalTimeAsISOString()});
    ...
    req.on('end', function() {
        try {
            ...
            if (parsedBody.value !== undefined) {
                message.value = parseFloat(parsedBody.value);
...
RS485Server.prototype.createHttpServer = function(port) {
    ...
    }).listen(config.nodePort, function () {
        console.log("Intranet server listening on: http://localhost:%s", config.nodePort);
    });
